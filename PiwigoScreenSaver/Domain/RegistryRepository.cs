...
namespace PiwigoScreenSaver.Domain
{
    ...
    public class RegistryRepository : ISettingsRepository
    {
        ...
        {
            return Convert.ToBase64String(ProtectedData.Protect(Encoding.Unicode.GetBytes(value),
                null, DataProtectionScope.CurrentUser));
        }

        private string Decrypt(string value)
        {
            return Encoding.Unicode.GetString(ProtectedData.Unprotect(Convert.FromBase64String(value),
                null, DataProtectionScope.CurrentUser));
        }
    }
}
